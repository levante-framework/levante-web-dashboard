#!/usr/bin/env node
/**
 * Build a US-only "place" boundary dataset using Census TIGER/Line Places.
 *
 * Goal: provide an in-between boundary for the US between tracts (very small) and counties (very large),
 * e.g. "City of Chicago" rather than a census tract vs Cook County.
 *
 * Output (per-state to keep runtime downloads reasonable):
 *   public/adm-packs/us/adm3-place/<state>.json.gz
 *
 * Usage:
 *   node scripts/us/build-us-place-boundaries.js --states=CA,OR,FL,NY,IL --year=2024 --precision=3 --maxPoints=300
 *
 * Notes:
 * - Uses the national Places file and filters features by STATEFP.
 * - Geometry is simplified by downsampling rings (no topo simplification).
 */
const fs = require('fs');
...
async function buildState(stateAbbr, year, precision, maxPoints) {
  ...
  const base = `https://www2.census.gov/geo/tiger/TIGER${year}/PLACE`;
  const zipName = `tl_${year}_${fips}_place.zip`;
  const url = `${base}/${zipName}`;

  const cacheDir = path.join(process.cwd(), 'data', 'tiger', `TIGER${year}`, 'PLACE');
  const zipPath = path.join(cacheDir, zipName);
  const extractDir = path.join(cacheDir, `tl_${year}_${fips}_place`);

  if (!fs.existsSync(zipPath)) {
    console.log(`⬇️  Downloading ${abbr} places (${year})...`);
  ...
  const shp = path.join(extractDir, `tl_${year}_${fips}_place.shp`);
  const dbf = path.join(extractDir, `tl_${year}_${fips}_place.dbf`);
  if (!fs.existsSync(shp) || !fs.existsSync(dbf)) {
    throw new Error(`Missing shp/dbf after unzip for ${abbr}`);
  }

  console.log(`🧹 Reading + minimizing ${abbr} places...`);
  const features = [];
  ...
  while (true) {
    ...
    features.push({
      ...
      geometry: geom,
      properties: {
        name: p.NAMELSAD || p.NAME || null, // e.g., "Chicago city"
        geoid: p.GEOID || null,
        state: abbr,
        statefp: p.STATEFP || fips,
        placefp: p.PLACEFP || null,
        lsad: p.LSAD || null
      }
    });
  }

  const outDir = path.join(process.cwd(), 'public', 'adm-packs', 'us', 'adm3-place');
  fs.mkdirSync(outDir, { recursive: true });
  const outPath = path.join(outDir, `${abbr.toLowerCase()}.json.gz`);
  const gz = zlib.gzipSync(Buffer.from(JSON.stringify({ type: 'FeatureCollection', features })), { level: 9 });
  fs.writeFileSync(outPath, gz);
  console.log(`💾 Wrote ${outPath} (${gz.length} bytes gz, ${features.length} places)`);
}
