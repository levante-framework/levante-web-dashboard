...
function buildUserPrompt(originalText, backTranslatedText, itemType) {
  ...
  return [
    ...
    '- Risk means the back-translated wording could change meaning, tone, or action enough that a child might misunderstand or choose a different response/action.',
    '- Focus on translation-quality risk (meaning/action drift), not speculative safety/compliance risk unless explicitly present in the text.',
    '',
    ...
    '',
    "issues: concise plain-English note of any meaning/action drift for child comprehension; if equivalent, return exactly 'No significant meaning loss detected.'",
    '',
    'Return JSON only.',
  ].join('\n');
}
