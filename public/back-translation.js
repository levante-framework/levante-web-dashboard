...
class Dashboard {
  constructor() {
    ...
    this.languages =
      window.CONFIG && window.CONFIG.languages
    ...
    this.embeddingAdvisoryEnabled =
      window.CONFIG?.embeddingAdvisoryEnabled !== false;
  ...
  refreshLanguagesFromConfig() {
    try {
      ...
      if (languageNames.length > 0 && this.currentLanguage) {
  ...
  isExcludedValidationItemId(itemId) {
    ...
    return (this.excludedValidationPrefixes || []).some((prefix) =>
  ...
  isSupportedCrowdinSourcePath(path) {
    ...
    return (
      ...
      contentGroup === "itembank_by_task" ||
      contentGroup === "surveys"
    );
  }
  ...
  sanitizeCrowdinRows(rows) {
    ...
    if (removed > 0) {
      console.log(
        `🧹 Removed ${removed} Z_ rows from imported translation data`,
      );
    }
    return filtered;
  }
  ...
  sanitizeValidationResultsStore() {
    ...
    if (removed > 0)
      console.log(
        `🧹 Removed ${removed} excluded legacy validation items from in-memory store`,
      );
  }
  ...
  getDisplayName(languageKey) {
    ...
    const langCode = String(lang?.lang_code || "")
      .trim()
      .toLowerCase();
    if (langCode === "fr-ca" && /^french$/i.test(String(languageKey || "")))
  ...
  async init() {
    this.setStatus("Loading dashboard...", "loading");
    ...
    try {
      ...
      this.setupTableControlListeners();
      ...
      this.setStatus("Select a language to load translations", "success");
    } catch (error) {
    ...
    if (selectEl) {
      ...
      selectEl.addEventListener("change", () => {
        ...
        this.loadData({ forceRefresh: false }).then(() => {
          if (this.currentLanguage) this.populateDataTable();
  ...
  async loadData(options = {}) {
    ...
    if (dataSource === "crowdin") {
      ...
      if (forceRefresh) {
        console.warn(
          "Crowdin refresh failed; keeping Crowdin mode without CSV fallback",
        );
      } else {
        ...
        const autoLoaded = await this.loadDataFromCrowdin({
          forceRefresh: true,
          suppressAlert: true,
          auto: true,
        });
        ...
        console.info(
          "No cached Crowdin data found yet; Crowdin mode requires successful refresh.",
        );
      }
      ...
      this.setStatus(
        'Could not load Crowdin translations. Shared validation/back-translation data will not align with CSV fallback. Please retry "Update Translations".',
        "error",
      );
  ...
  updateDataSourceLabel(source) {
    ...
    if (!label) return;
    const displaySource =
      source !== undefined && source !== null ? source : this.currentDataSource;
    if (displaySource) {
  ...
  applyCrowdinImportChangeFlags(previousRows, nextRows) {
    ...
    if (!prevData.length || !nextData.length)
      return { changedEntries: 0, changedItems: 0 };
    ...
    nextData.forEach((nextRow) => {
      const itemId = String(
        nextRow?.item_id || nextRow?.identifier || "",
      ).trim();
      if (!itemId) return;
      ...
      const sourceChanged =
        this.normalizeComparableText(prevRow?.en) !==
        this.normalizeComparableText(nextRow?.en);
      ...
      langCandidates.forEach((langCode) => {
        ...
        if (!lang) return;
        const existingLangKey = this.findExistingValidationLangKey(
          byItem,
          lang,
        );
        ...
        const prevTranslation = this.getTranslationTextForLanguage(
          prevRow,
          lang,
        );
        const nextTranslation = this.getTranslationTextForLanguage(
          nextRow,
          lang,
        );
        const translationChanged =
          this.normalizeComparableText(prevTranslation) !==
          this.normalizeComparableText(nextTranslation);
        if (!sourceChanged && !translationChanged) return;

        const changeKind =
          sourceChanged && translationChanged
            ? "source+translation"
            : sourceChanged
              ? "source"
              : "translation";
  ...
  normalizeCrowdinCacheRows(rows) {
    ...
    for (let i = 0; i < data.length; i++) {
      const row =
        data[i] && typeof data[i] === "object" ? { ...data[i] } : null;
      if (!row) continue;
      const itemId = String(
  ...
  attachDerivedDisplayFields(rows) {
    (rows || []).forEach((item) => {
      ...
      const displayItemId = rawId.includes("::")
        ? rawId.split("::").pop()
        : rawId;
      const compactItemId =
        displayItemId.length > 36
          ? `${displayItemId.slice(0, 33)}...`
          : displayItemId;
      const meta = {
        displayItemId,
        compactItemId,
  ...
  getLanguageAliasCodes(langCode) {
    ...
    const base = normalizedHyphen.includes("-")
  ...
  isManualApprovedEntry(entry) {
    ...
    const source = String(result.scoreSource || "")
      .trim()
      .toLowerCase();
  ...
  normalizeValidationPathKey(rawPath) {
    ...
    if (
      parts.length >= 2 &&
      /^([a-z]{2}(?:-[a-z0-9]{2,4})?)$/i.test(parts[0]) &&
      parts[1].toLowerCase() === "main"
    ) {
  ...
  getValidationEntry(itemOrId, langCode) {
    ...
    const sourcePathHints = (() => {
      ...
      try {
        return this.getItemSourcePaths(itemOrId)
          .map((p) => this.normalizeValidationPathKey(p))
          .filter(Boolean);
      } catch (_) {
        return [];
      }
    ...
    const normalizedRaw = rawItemId
      .replace(/\.xliff(?=::|$)/gi, ".xlf")
      .replace(/\.xlf(?=::|$)/gi, ".xliff");
    ...
    const tailSlashLeaf = tailPrimary.includes("/")
      ? tailPrimary.split("/").pop()
      : tailPrimary;
    ...
    Array.from(exactNeedles).forEach((needle) => {
      if (needle.includes(".xliff"))
        extSwapNeedles.add(needle.replace(/\.xliff(?=::|$)/g, ".xlf"));
      if (needle.includes(".xlf"))
        extSwapNeedles.add(needle.replace(/\.xlf(?=::|$)/g, ".xliff"));
    });
    ...
    if (suffixMatches.length === 1) {
      ...
      const byPath = suffixMatches.filter((storedKey) => {
        ...
        return (
          normalizedPath &&
          sourcePathHints.some(
            (hint) =>
              normalizedPath.endsWith(hint) || hint.endsWith(normalizedPath),
          )
        );
      });
  ...
  getEmbeddingAdvisoryUrls() {
    ...
    return [
      "/api/embedding-advisory",
      "./data/validation/embedding-advisory.json",
    ];
  }
  ...
  async loadEmbeddingAdvisory() {
    ...
    for (const url of urls) {
      try {
        ...
        console.log(
          `✅ Loaded embedding advisory entries: ${entries.length} from ${url}`,
        );
        return true;
      } catch (error) {
        console.log(
          `Embedding advisory load skipped for ${url}:`,
          error?.message || error,
        );
      }
    }
    console.log(
      "No embedding advisory artifact found; continuing without advisory data.",
    );
    return false;
  }
  ...
  isPathRelevantForLanguage(path, language) {
    ...
    const isLanguageSegment = /^[a-z]{2}(?:-[a-z0-9]{2,4})?$/i.test(
      firstSegment,
    );
    // Shared dashboard CSVs should be available in every language tab.
    if (
      normalizedPath.toLowerCase().includes("main/dashboard/") &&
    ...
    const aliasSet = new Set(
      this.getLanguageAliasCodes(langCode).map((c) => c.toLowerCase()),
    );
  ...
  refreshFileFilterOptions(language, baseRows) {
    ...
    if (!selectEl) return;
    const previousValue =
      this.fileFilterByLanguage.get(language) || selectEl.value || "all";
  ...
  getFilteredItemsForLanguage(language) {
    const allowedIds =
      typeof window.getReviewTableAllowedItemIds === "function"
        ? window.getReviewTableAllowedItemIds()
        : null;
    if (!allowedIds) return this.data;
    return this.data.filter((item) =>
      allowedIds.has(String(item.item_id || item.identifier || "")),
    );
  }

  getApprovalFilterForLanguage(language) {
    const raw = String(this.approvalFilterByLanguage.get(language) || "all")
      .trim()
      .toLowerCase();
  ...
  setApprovalFilterBusy(language, isBusy) {
    ...
    if (!lang) return;
    document
      .querySelectorAll("#languagePanel .approval-filter-btn")
      .forEach((btn) => {
        const mode = String(btn.dataset.filter || "")
          .trim()
          .toLowerCase();
  ...
  applyApprovalFilterForLanguage(language, filterMode) {
    ...
    if (!lang) return;
    const normalized = String(filterMode || "all")
      .trim()
      .toLowerCase();
    const next =
      normalized === "approved" || normalized === "not-approved"
        ? normalized
        : "all";
    ...
    const shouldSync =
      target !== "all" &&
      !!langCode &&
      typeof this.loadFromSharedStorage === "function";
    if (!shouldSync) {
  ...
  applyApprovalFilterToRows(rows, langCode, language) {
    const filterMode = this.getApprovalFilterForLanguage(
      language || this.currentLanguage,
    );
    if (filterMode === "all") return rows;
    const normalizedLang = String(langCode || "").trim();
    return (rows || []).filter((item) => {
      const storedResult = this.getValidationEntryForDisplay(
        item,
        normalizedLang,
      );
  ...
  applyApprovalFilterToRowsWithCache(rows, langCode, language, entryCache) {
    ...
    const normalizedLang = String(langCode || "").trim();
    return (rows || []).filter((item) => {
      const storedResult = this.getCachedValidationEntry(
        entryCache,
        item,
        normalizedLang,
      );
  ...
  computeValidationSummaryCountsForRows(rows, langCode, entryCache = null) {
    ...
    const isSourceEnglishTab =
      String(normalizedLang).split("-")[0].toLowerCase() === "en";
    (rows || []).forEach((item) => {
      ...
      if (!requiresRevalidation && this.isManualApprovedEntry(storedResult))
        approved++;

      const translatedText = this.extractTextForItem(
        item || {},
        normalizedLang,
      );
  ...
  setTableLoadingState(language, isLoading, loadedCount = 0, totalCount = 0) {
    if (!this.currentLanguage) return;
    const statusEl = document.getElementById("tableLoading");
    const contentEl = document.getElementById("languagePanel");
    const controls = contentEl
      ? Array.from(
          contentEl.querySelectorAll(
            ".sort-controls .btn, .sort-controls select",
          ),
        )
      : [];
    controls.forEach((el) => {
    ...
    statusEl.innerHTML =
      '<i class="fas fa-check-circle" style="font-size:1.05em;"></i> Table Loaded';
  }
  ...
  ensureLanguageFullyRendered(language) {
    ...
    if (!state || state.completed) return;
    while (this.appendRenderBatch(state, 250)) {
  ...
  loadFflate() {
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () =>
        reject(new Error("Could not load fflate from cdn.jsdelivr.net"));
  ...
  async loadDataFromCrowdin(options = {}) {
    ...
    try {
      if (!forceRefresh) {
        ...
        this.setStatus(
          'No Crowdin cache found yet. Using CSV fallback now. Click "Update Translations" to fetch Crowdin and seed cache.',
          "warning",
        );
        return false;
      }
      ...
      const response = await fetch("/api/crowdin-approved-translations", {
        cache: "no-cache",
      });
      const contentType = (
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (response.status === 504) {
            msg =
              "Crowdin build took too long (timeout). Try again in a minute.";
          } else if (response.status === 500) {
            msg =
              "Server error (500). Often caused by: Crowdin API token not set in Vercel (CROWDIN_API_TOKEN), or the build timed out.";
          }
        }
        throw new Error(msg);
      }
      ...
      if (contentType.includes("application/json")) {
        ...
        if (!zipRes || !zipRes.ok) {
          ...
          if (!proxyRes.ok)
            throw new Error(`ZIP download failed: ${proxyRes.status}`);
      ...
      if (!fflateLib || !fflateLib.unzipSync)
        throw new Error(
          "fflate library not loaded. Allow cdn.jsdelivr.net or check your network.",
        );
      ...
      this.logPerf(
        "Crowdin zip unzip",
        unzipStart,
        `files=${Object.keys(unzipped).length}`,
      );
      ...
      this.logPerf(
        "Crowdin CSV/XLIFF merge",
        mergeStart,
        `rows=${Array.isArray(merged) ? merged.length : 0}`,
      );
      if (!Array.isArray(merged) || merged.length === 0)
        throw new Error("No CSV or XLIFF data in Crowdin export");
      ...
      try {
        ...
        if (hiddenSet && hiddenSet.size) {
          ...
          if (visibleRows.length === 0) {
            ...
            if (hiddenExcludedCount > 0) {
              console.log(
                `🙈 Excluded ${hiddenExcludedCount} hidden Crowdin string(s) from import`,
              );
            }
          }
        }
        ...
        console.warn(
          "Hidden-string filter skipped:",
          hiddenErr?.message || hiddenErr,
        );
      }
      ...
      this.logPerf(
        "Crowdin setLoadedData",
        setDataStart,
        `rows=${this.data.length}`,
      );
      const importChangeSummary = this.applyCrowdinImportChangeFlags(
        previousRows,
        this.data,
      );
      ...
      if (importChangeSummary.changedEntries > 0) {
        ...
        this.setStatus(
          `Loaded ${this.data.length} items from ${source}.${hiddenNote}`,
          "success",
        );
      }
      ...
      this.setStatus(
        `Crowdin refresh failed (${error.message}). Using CSV fallback for now.`,
        "warning",
      );
  ...
  async fetchHiddenCrowdinIdentifiers() {
    try {
      const res = await fetch("/api/crowdin-hidden-strings", {
        cache: "no-cache",
      });
      if (!res.ok) return null;
      const json = await res.json();
      const list = Array.isArray(json.hiddenIdentifiers)
        ? json.hiddenIdentifiers
        : [];
      const set = new Set();
  ...
  isHiddenCrowdinRow(row, hiddenSet) {
    ...
    if (id.includes("::")) {
      ...
      if (tail.includes("/"))
        candidates.add(tail.split("/").pop().toLowerCase());
    }
  ...
  async loadCrowdinDataFromCache() {
    ...
    try {
      ...
      this.logPerf(
        "Crowdin cache localStorage read",
        localStart,
        raw ? "hit" : "miss",
      );
      if (raw) {
        ...
        this.logPerf(
          "Crowdin cache localStorage parse",
          parseStart,
        ...
        if (cached && Array.isArray(cached.data) && cached.data.length > 0) {
          ...
          const schemaMatches =
            String(cached.cacheSchemaVersion || "") ===
            CROWDIN_CACHE_SCHEMA_VERSION;
          const canMigrateLegacy =
            String(cached.cacheFormat || "") === "crowdin-full-merge";
          if (!schemaMatches && !canMigrateLegacy) {
            console.warn(
              "Ignoring outdated non-migratable Crowdin cache from localStorage; refreshing with current schema.",
            );
          } else if (!this.hasUsableTranslationsDataset(normalizedRows)) {
            console.warn(
              "Ignoring invalid cached Crowdin data from localStorage (missing base/translated text).",
            );
          } else {
            const setDataStart = this.perfNow();
            this.setLoadedData(normalizedRows);
            this.logPerf(
              "Crowdin cache setLoadedData (localStorage)",
              setDataStart,
              `rows=${this.data.length}`,
            );
            this.crowdinFilesUsed = Array.isArray(cached.crowdinFilesUsed)
              ? cached.crowdinFilesUsed
              : null;
            ...
            const cachedAt = cached.cachedAt
              ...
              : "previous session";
            this.setStatus(
              `Loaded ${this.data.length} items from cached Crowdin data (${cachedAt}, CSV + XLIFF)`,
              "success",
            );
            this.updateDataSourceLabel(
              `cached Crowdin data (retrieved ${cachedAt})`,
            );
            if (
              !schemaMatches ||
              normalizedRows.length !== cached.data.length
            ) {
              console.log("♻️ Migrating cached Crowdin data to latest schema.");
              await this.cacheCrowdinDataLocally({
                rows: normalizedRows,
                files: Array.isArray(cached.crowdinFilesUsed)
                  ? cached.crowdinFilesUsed
                  : this.crowdinFilesUsed || [],
              });
            }
    ...
    try {
      ...
      if (!schemaMatches && !canMigrateLegacy) {
        console.warn(
          "Ignoring outdated non-migratable Crowdin cache from IndexedDB; refreshing with current schema.",
        );
        return false;
      }
      if (!this.hasUsableTranslationsDataset(normalizedRows)) {
        console.warn(
          "Ignoring invalid cached Crowdin data from IndexedDB (missing base/translated text).",
        );
        return false;
      }
      const setDataStart = this.perfNow();
      this.setLoadedData(normalizedRows);
      this.logPerf(
        "Crowdin cache setLoadedData (IndexedDB)",
        setDataStart,
        `rows=${this.data.length}`,
      );
      this.crowdinFilesUsed = Array.isArray(cached.crowdinFilesUsed)
  ...
  async cacheCrowdinDataLocally(options = {}) {
    try {
      const files = Array.isArray(options?.files)
        ? options.files
        : this.crowdinFilesUsed || [];
      const rowsForCache = Array.isArray(options?.rows)
        ? options.rows
        : this.data || [];
      ...
      const payload = {
        ...
        cacheIncludes: {
          csv: files.filter((f) => String(f).toLowerCase().endsWith(".csv"))
            .length,
      ...
      try {
        ...
        if (
          !String(storageError && storageError.name).includes(
            "QuotaExceededError",
          )
        ) {
          console.warn(
            "Could not write Crowdin cache to localStorage:",
            storageError,
          );
        }
      }
      ...
      if (!localSaved) {
        console.warn(
          "Crowdin cache exceeded localStorage quota; using IndexedDB cache",
        );
      }
    } catch (error) {
  ...
  parseCrowdinZipToMerged(unzipped) {
    ...
    function langFromFirstSegment(path) {
      ...
      return (
        LANG_ID_TO_CODE[first] ||
        LANG_ID_TO_CODE[lower] ||
        (lower === "en-us" ? "en" : first)
      );
    }
    ...
    function normalizeItem(item) {
      const itemId =
        ...
        null;
      const task =
        item.task || item.labels || item.category || item.type || "general";
      const en = (
        item.en ||
        item.source ||
        item.source_phrase ||
    ...
    function deriveTaskAndTypeFromPath(path) {
      ...
      function prettifyLabel(label) {
        ...
        return raw
          .replace(/\.[a-z0-9]+$/i, "")
          .replace(/[_-]+/g, " ")
          .replace(/\b(short|newkeys?)\b/gi, "")
          .replace(/\s+/g, " ")
          .trim()
          .replace(/\b\w/g, (c) => c.toUpperCase());
      }
      const itembankMatch = compact.match(
        /(?:^|\/)itembank_by_task\/([^/]+)\.xli?ff$/i,
      );
      if (itembankMatch) {
    ...
    function normalizeLangCodeFromHeader(header, fallback = "") {
      ...
      if (!token) return "";
      const mapped =
        LANG_ID_TO_CODE[token] || LANG_ID_TO_CODE[token.toLowerCase()] || token;
      ...
      try {
        ...
        function getLocalName(node) {
          ...
          return (
            node.localName ||
            node.baseName ||
            (node.tagName && String(node.tagName).split(":").pop()) ||
            ""
          ).toLowerCase();
        }
        ...
        if (transUnits.length) {
          transUnits.forEach((tu) => {
            ...
            const resname =
              tu.getAttribute("resname") || tu.getAttribute("name") || "";
            const src =
              tu.querySelector("source") ||
              childByLocalName(tu, "source") ||
              allDescendantsByLocalName(tu, "source")[0];
            const tgt =
              tu.querySelector("target") ||
              childByLocalName(tu, "target") ||
              allDescendantsByLocalName(tu, "target")[0];
            units.push({
              id,
              resname,
              source: getTextFromNode(src),
              target: getTextFromNode(tgt),
            });
          });
          return units;
        }
        ...
        unitEls.forEach((unit) => {
          ...
          const resname =
            unit.getAttribute("name") || unit.getAttribute("resname") || "";
          const seg =
            unit.querySelector("segment") ||
            childByLocalName(unit, "segment") ||
            allDescendantsByLocalName(unit, "segment")[0];
          const src = seg
            ? seg.querySelector("source") ||
              childByLocalName(seg, "source") ||
              allDescendantsByLocalName(seg, "source")[0]
            : unit.querySelector("source") ||
              childByLocalName(unit, "source") ||
              allDescendantsByLocalName(unit, "source")[0];
          const tgt = seg
            ? seg.querySelector("target") ||
              childByLocalName(seg, "target") ||
              allDescendantsByLocalName(seg, "target")[0]
            : unit.querySelector("target") ||
              childByLocalName(unit, "target") ||
              allDescendantsByLocalName(unit, "target")[0];
          units.push({
            id,
            resname,
            source: getTextFromNode(src),
        ...
        console.warn("XLIFF parse error:", e);
      }
      return units;
    }
    ...
    for (const [path, data] of Object.entries(unzipped)) {
      ...
      if (lowerPath.startsWith("main/dashboard/")) {
        ...
        objects.forEach((row, index) => {
          ...
          langHeaders.forEach((headerName) => {
            ...
            if (!byLang[langCode])
              byLang[langCode] = {
                headers: ["identifier", langCode],
                objects: [],
              };
            const existingIdx = byLang[langCode].objects.findIndex(
              (r) => getIdFromRow(r, byLang[langCode].headers) === stableId,
            );
            const obj = {
              identifier: stableId,
              [langCode]: textValue,
      ...
      if (!byLang[lang]) {
        ...
        cur.objects.forEach((r) => {
          const id = getIdFromRow(r, cur.headers);
          merged.set(id, r);
        });
        objects.forEach((r) => {
        ...
        byLang[lang] = {
          headers: cur.headers,
          objects: Array.from(merged.values()),
        };
      }
    }
    ...
    for (const [path, data] of Object.entries(unzipped)) {
      ...
      units.forEach((u) => {
        ...
        const existingEnIdx = byLang.en.objects.findIndex(
          (r) => getIdFromRow(r, byLang.en.headers) === stableId,
        );
        if (existingEnIdx === -1) {
          ...
          const existingPath = String(
            byLang.en.objects[existingEnIdx]._path || "",
          );
        ...
        if (!byLang[targetLang])
          byLang[targetLang] = { headers: tgtHeaders, objects: [] };
        const hasTgt = byLang[targetLang].objects.some(
          (r) => getIdFromRow(r, byLang[targetLang].headers) === stableId,
        );
    ...
    function getRowByLang(lang, id) {
      if (String(id).startsWith("_row_")) {
        ...
        if (byLang[langPart] && byLang[langPart].objects[i] !== undefined)
          return byLang[langPart].objects[i];
        return null;
      }
      return (
        (byLang[lang] &&
          byLang[lang].objects.find(
            (r) => getIdFromRow(r, byLang[lang].headers) === id,
          )) ||
        null
      );
    }
    return idList.map((id, idx) => {
      const baseRow =
        getRowByLang(baseLang, id) ||
        Object.keys(byLang)
          .map((lang) => getRowByLang(lang, id))
          .find(Boolean);
      const row = baseRow || {};
      const enRow = byLang.en ? getRowByLang("en", id) : null;
      const enText = enRow
        ? enRow.en || enRow.source || enRow.english || enRow.text || ""
        : row.en || row.source || row.english || row.text || "";
      const out = {
        ...row,
        item_id: id,
        labels: row.task || row.labels || "general",
      ...
      Object.keys(byLang).forEach((lang) => {
        ...
        if (other) {
          ...
          const textKey = otherHeaders.find(
            (h) =>
              h &&
              !/identifier|item_id|^id$/i.test(String(h)) &&
              (h === lang ||
                h.replace(/_/g, "-") === lang ||
                h.toLowerCase() === lang.toLowerCase()),
          );
          if (textKey) out[lang] = other[textKey] ?? "";
          else
            out[lang] =
              other.en || other.source || other.english || other.text || "";
        }
      });
      out._sourcePaths = [...new Set(sourcePaths)];
      if (out.labels === "general") {
        const derived = deriveTaskAndTypeFromPath(
          out._sourcePaths[0] || out._path || "",
        );
        out.labels = derived.task;
        if (!out.contentType || out.contentType === "general")
          out.contentType = derived.contentType;
      }
  ...
  async loadDataFromCSV() {
    try {
      ...
      const configuredSurveys =
        cfg.remoteSurveysCSV ||
        (configuredItemBank &&
        configuredItemBank.endsWith("/item-bank-translations.csv")
          ? configuredItemBank.replace(
      ...
      if (loadedBatches.length > 0) {
          ...
          batch.rows.forEach((row, index) => {
            ...
            const normalizedRow = {
              ...
              ...(inferredSourcePath
                ? {
                    _path:
                      String(row?._path || "").trim() || inferredSourcePath,
                  }
                : {}),
              ...(inferredSourcePath || rowSourcePaths.length
                ? {
                    _sourcePaths: [
                      ...new Set([
                        ...(rowSourcePaths || []),
                        ...(inferredSourcePath ? [inferredSourcePath] : []),
                      ]),
                    ],
                  }
                : {}),
            ...
            const rawId = String(
              normalizedRow.item_id ||
                normalizedRow.identifier ||
                normalizedRow.id ||
                normalizedRow.ID ||
                normalizedRow.Item_ID ||
                "",
            ...
            const mergedSourcePaths = [
              ...new Set([
                ...(Array.isArray(existing._sourcePaths)
                  ? existing._sourcePaths
                  : []),
                ...(Array.isArray(normalizedRow._sourcePaths)
                  ? normalizedRow._sourcePaths
                  : []),
              ]),
            ];
            mergedById.set(key, {
        ...
        console.log(
          "CSV files used:",
          loadedBatches.map((b) => b.url),
        );
        this.setStatus(
          `Loaded ${this.data.length} items from ${source}`,
          "success",
        );
        this.updateDataSourceLabel(source);
        this.cacheDataLocally(null);
      ...
      if (!csvText) {
        try {
          ...
          const localResponse = await fetch(
            "./translation_text/complete_translations.csv",
          );
          if (localResponse.ok) {
      ...
      try {
        ...
        if (cachedData) {
          ...
          this.setStatus(
            `Loaded ${this.data.length} items from cache (offline)`,
            "success",
          );
  ...
  parseCSV(csvText) {
    ...
    console.log(
      `🔧 Robust CSV Parser: Parsed ${data.length} rows successfully`,
    );
  ...
  parseCSVWithEmbeddedNewlines(csvText) {
    ...
    console.log(
      `🔧 Robust CSV Parser: ${validRows.length} valid rows after filtering`,
    );
    return validRows;
  }
    ...
    return [
      {
        ...
        "es-CO": "Hola, bienvenido a la prueba.",
        de: "Hallo, willkommen zum Test.",
        "fr-CA": "Bonjour, bienvenue au test.",
        nl: "Hallo, welkom bij de test.",
      },
      {
        item_id: "sample_2",
        labels: "math",
        en: "Count the numbers.",
        "es-CO": "Cuenta los números.",
        de: "Zähle die Zahlen.",
        "fr-CA": "Comptez les nombres.",
        nl: "Tel de nummers.",
      },
      {
        item_id: "sample_3",
        ...
        "es-CO": "¿Qué es esta palabra?",
        de: "Was ist dieses Wort?",
        "fr-CA": "Quel est ce mot?",
        nl: "Wat is dit woord?",
      },
    ];
  }
  ...
  async loadValidationResults() {
    try {
      ...
      if (storedResults) {
        ...
        console.log(
          `✅ Loaded ${Object.keys(this.validation_results).length} validation results from localStorage`,
        );
      }
      ...
      if (!sharedLoaded && !storedResults) {
        console.log(
          "📝 No shared/local validation data found, checking static JSON file...",
        );
        try {
          ...
          if (jsonResponse.ok) {
            ...
            if (jsonData.validation_results) {
              ...
              this.normalizeValidationResultsLanguageKeys();
              console.log(
                `✅ Loaded ${Object.keys(this.validation_results).length} validation results from JSON file`,
              );
              console.log(
                `📅 File exported: ${jsonData.metadata?.exported_at || "Unknown date"}`,
              );
            }
          }
        } catch (jsonError) {
          console.log(
            "📝 No validation_results.json file found, starting fresh",
          );
        }
      }
    } catch (error) {
  ...
  updateValidationSaveBaseline() {
    this.validationSaveBaseline = this.stableStringifyValidationResults(
      this.validation_results,
    );
    try {
...
<div id="unsavedValidationModal" class="modal" style="display: none;">
    ...
    if (save) {
      save.addEventListener("click", async () => {
        ...
        try {
          ...
          this.setStatus(
            "Could not save validation results. Fix any issues and try again, or choose Stay or Discard.",
            "error",
          );
        } catch (err) {
  ...
  hasUnsavedValidationChanges() {
    ...
    return (
      this.stableStringifyValidationResults(this.validation_results) !==
      this.validationSaveBaseline
    );
  }
  ...
  setupUnsavedValidationWarning() {
    if (
      typeof window === "undefined" ||
      this._unsavedValidationBeforeUnloadBound
    )
      return;
  ...
  async saveValidationResults(options = {}) {
    ...
    try {
      console.log(
        "💾 Saving validation results to localStorage and shared storage...",
      );
      this.sanitizeValidationResultsStore();
      this.normalizeValidationResultsLanguageKeys();
      ...
      try {
        localStorage.setItem(
          "validation_results",
          JSON.stringify(this.validation_results),
        );
      } catch (localError) {
        if (
          localError &&
          (localError.name === "QuotaExceededError" ||
            String(localError.message || "").includes("quota"))
        ) {
          console.warn(
            "⚠️ localStorage quota exceeded, trying compact validation snapshot",
          );
          ...
          try {
            ...
            console.warn(
              "⚠️ Compact local snapshot also failed:",
      ...
      console.log(
        `✅ Saved ${Object.keys(this.validation_results).length} items with ${totalValidations} total validations`,
      );
  ...
  async saveToSharedStorage(options = {}) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          "✅ Successfully saved to shared storage:",
          result.metadata,
        );
        if (!silent)
          this.setStatus(
            "💾 Validation results saved to shared session storage for team access",
            "success",
          );
        return true;
      } else {
        console.warn(
          "⚠️ Failed to save to shared storage, but localStorage backup is available",
        );
        return false;
      }
    } catch (error) {
  ...
  buildCompactValidationResultsSnapshot() {
    ...
    Object.keys(source).forEach((itemId) => {
      ...
      Object.keys(byLang).forEach((langCode) => {
        ...
        if (
          typeof r.manualApprovalUpdatedAt === "string" &&
          r.manualApprovalUpdatedAt
        )
          entry.manualApprovalUpdatedAt = r.manualApprovalUpdatedAt;
        if (r.needsReview === true) entry.needsReview = true;
        if (typeof r.reason === "string" && r.reason)
          entry.reason = r.reason.slice(0, 400);
        if (typeof r.reviewUpdatedAt === "string" && r.reviewUpdatedAt)
          entry.reviewUpdatedAt = r.reviewUpdatedAt;
        if (r.requiresRevalidation === true) entry.requiresRevalidation = true;
        if (typeof r.changeKind === "string" && r.changeKind)
          entry.changeKind = r.changeKind;
        if (typeof r.changeDetectedAt === "string" && r.changeDetectedAt)
          entry.changeDetectedAt = r.changeDetectedAt;
        if (typeof r.lastSeenSourceHash === "string" && r.lastSeenSourceHash)
          entry.lastSeenSourceHash = r.lastSeenSourceHash;
        if (
          typeof r.lastSeenTranslationHash === "string" &&
          r.lastSeenTranslationHash
        )
          entry.lastSeenTranslationHash = r.lastSeenTranslationHash;
        if (typeof r.notes === "string" && r.notes)
          entry.notes = r.notes.slice(0, 160);
        if (typeof r.backTranslation === "string" && r.backTranslation)
          entry.backTranslation = r.backTranslation.slice(0, 1000);
        if (typeof r.aiUsed === "boolean") entry.aiUsed = r.aiUsed;
        if (Number.isFinite(Number(r.aiScore)))
          entry.aiScore = Number(r.aiScore);
        if (Number.isFinite(Number(r.baselineScore)))
          entry.baselineScore = Number(r.baselineScore);
        if (typeof r.manualOverridePreviousScore === "number")
          entry.manualOverridePreviousScore = r.manualOverridePreviousScore;
        if (r.manualOverridePreviousSource)
          entry.manualOverridePreviousSource = r.manualOverridePreviousSource;
        compactByLang[langCode] = entry;
      });
      if (Object.keys(compactByLang).length > 0)
        compact[itemId] = compactByLang;
    });
    return compact;
  }
  ...
  async loadFromSharedStorage(languageOverride = "", options = {}) {
    try {
      ...
      if (
        typeof window !== "undefined" &&
        typeof window.flushValidationAutoSave === "function"
      ) {
        try {
          await window.flushValidationAutoSave();
        } catch (_) {
          /* best effort */
        }
      }

      const configuredRemoteSharedEndpoint = String(
        window.CONFIG?.sharedValidationEndpoint || "",
      ).trim();
      ...
      const requestedLangKey =
        this.getValidationLanguageLoadKey(currentLangCode);
      if (
        !forceReload &&
        requestedLangKey &&
        this.loadedValidationLanguageCodes.has(requestedLangKey)
      ) {
        return false;
      }
      ...
      if (
        configuredRemoteSharedEndpoint &&
        !endpointCandidates.includes(configuredRemoteSharedEndpoint)
      ) {
      ...
      for (const endpoint of endpointCandidates) {
        ...
        if (scopedEndpoint)
          attempts.push({ url: scopedEndpoint, scoped: true });
        ...
        for (const attempt of attempts) {
          ...
          try {
            ...
            const sharedResultsRaw =
              data?.validation_results || result?.validation_results || {};
            if (!sharedResultsRaw || typeof sharedResultsRaw !== "object")
              continue;
            ...
            console.log(
              `Shared validation load skipped for ${attempt.url}:`,
      ...
      if (payloadCandidates.length > 0) {
        ...
        Object.keys(sharedResults).forEach((itemId) => {
          ...
          if (!localResults[itemId]) {
            ...
            Object.keys(sharedResults[itemId]).forEach((lang) => {
              ...
              const merged = {
                ...(localValidation && typeof localValidation === "object"
                  ? localValidation
                  : {}),
                ...(sharedValidation && typeof sharedValidation === "object"
                  ? sharedValidation
                  : {}),
              };
              ...
              if (localValidation && typeof localValidation === "object") {
                ...
                const sharedRev = String(
                  (sharedValidation && sharedValidation.reviewUpdatedAt) || "",
                );
                if (
                  localValidation.needsReview === true &&
                  (!sharedRev || localRev > sharedRev)
                ) {
                  merged.needsReview = true;
                  if (localRev) merged.reviewUpdatedAt = localRev;
                  if (localValidation.reason != null && !merged.reason)
                    merged.reason = localValidation.reason;
                }
                const localAppr = String(
                  localValidation.manualApprovalUpdatedAt || "",
                );
                const sharedAppr = String(
                  (sharedValidation &&
                    sharedValidation.manualApprovalUpdatedAt) ||
                    "",
                );
                if (
                  localValidation.manualApproved === true &&
                  (!sharedAppr || localAppr > sharedAppr)
                ) {
        ...
        if (requestedLangKey)
          this.loadedValidationLanguageCodes.add(requestedLangKey);
        console.log(
          `✅ Loaded shared validation results: ${Object.keys(sharedResults).length} items from ${bestPayload.endpoint} (${bestPayload.source})`,
        );
        const sourceLabel =
          this.sharedValidationSource === "gcs"
            ? "shared bucket (GCS)"
            : this.sharedValidationSource === "memory"
              ? "session memory fallback"
              : this.sharedValidationSource;
        this.setStatus(
          `🌐 Loaded validation results from ${sourceLabel}`,
          this.sharedValidationSource === "memory" ? "warning" : "success",
        );
        this.updateValidationSaveBaseline();
        return true;
      }
  ...
  applyStoredValidationResults() {
    ...
    console.log(
      "⚠️ applyStoredValidationResults is deprecated, use applyStoredValidationResultsForCurrentLanguage",
    );
  ...
  applyStoredValidationResultsForCurrentLanguage() {
    ...
    const currentLangCode =
      this.languages[this.currentLanguage]?.lang_code || "unknown";
    console.log(
      `🎯 Applying stored validation results for ${this.currentLanguage} (${currentLangCode})... [SKIPPED - pre-computed in HTML]`,
    );
    if (typeof updateValidationSummary === "function") {
      updateValidationSummary();
    }
  ...
  storeValidationResult(itemId, langCode, score, notes = "") {
    ...
    this.validation_results[itemId][preferredLangCode] = {
      ...
      manualApproved:
        existing && existing.manualApproved === true ? true : false,
      manualApprovalUpdatedAt:
        existing && existing.manualApprovalUpdatedAt
          ? existing.manualApprovalUpdatedAt
          : "",
      needsReview:
        existing && existing.needsReview !== undefined
          ? existing.needsReview
          : false,
      reason: existing && existing.reason !== undefined ? existing.reason : "",
      backTranslation:
        existing && existing.backTranslation !== undefined
          ? existing.backTranslation
          : "",
      reviewUpdatedAt:
        existing && existing.reviewUpdatedAt !== undefined
          ? existing.reviewUpdatedAt
          : "",
    };
    console.log(
      `📝 Stored validation result: ${itemId}[${preferredLangCode}] = ${score}%`,
    );
  }

  updateValidationUI(itemId, langCode, score, notes) {
    const currentCode = this.getCurrentLanguageCode();
    if (!currentCode || !this.currentLanguage) return;
    const aliases = this.getLanguageAliasCodes(currentCode);
    if (!aliases.includes(String(langCode || "").trim())) return;

    const languageTab = document.getElementById("tableContent");
    if (!languageTab) return;

    const indicator = languageTab.querySelector(`[data-item-id="${itemId}"]`);
    if (!indicator) {
      console.warn(
        `Indicator not found for ${itemId} in current language table`,
      );
      return;
    }
    ...
    if (!scoreBadge) {
      ...
      scoreBadge.style.cssText =
        "font-size: 10px; font-weight: bold; margin-left: 4px; opacity: 0.9;";
  ...
  populateLanguageDropdown() {
    ...
    const previous = this.currentLanguage || dropdown.value || "";
    const languageNames = Object.keys(this.languages || {}).sort((a, b) =>
      String(this.getDisplayName(a) || a).localeCompare(
        String(this.getDisplayName(b) || b),
      ),
    );

    dropdown.innerHTML = '<option value="" hidden>Select language</option>';
    languageNames.forEach((languageKey) => {
      const option = document.createElement("option");
      option.value = languageKey;
      option.textContent = this.getDisplayName(languageKey);
      dropdown.appendChild(option);
    });

    if (previous && this.languages[previous]) {
  ...
  bindLanguageDropdown() {
    ...
    dropdown.addEventListener("change", () => {
      const value = String(dropdown.value || "").trim();
      if (!value) {
        void this.clearLanguageSelection();
        return;
      }
  ...
  updateLanguagePanelHeader() {
    const panel = document.getElementById("languagePanel");
    const emptyEl = document.getElementById("languagePanelEmpty");
    const flagEl = document.getElementById("languagePanelFlag");
    const nameEl = document.getElementById("languagePanelName");
    const codeEl = document.getElementById("languageCodeLabel");
    ...
    const displayLanguage = this.getDisplayName(this.currentLanguage);
    const langConfig = this.languages[this.currentLanguage] || {};
    if (panel) panel.style.display = "";
    if (emptyEl) emptyEl.style.display = "none";
    if (flagEl) flagEl.innerHTML = this.getFlagForLanguage(displayLanguage);
    if (nameEl) nameEl.textContent = displayLanguage;
    if (codeEl) codeEl.textContent = langConfig.lang_code || "—";
  }
  ...
  clearLanguageSelection() {
    ...
    this.updateLanguagePanelHeader();
    if (typeof window.setValidationSummaryCounts === "function") {
      window.setValidationSummaryCounts({
        good: 0,
        warning: 0,
        error: 0,
        needsReview: 0,
  ...
  async selectLanguage(languageKey) {
    if (!languageKey || !this.languages[languageKey]) return;
    if (languageKey === this.currentLanguage) return;

    if (this.currentLanguage && this.hasUnsavedValidationChanges()) {
    ...
    const langConfig = this.languages[languageKey];
    const label = langConfig
      ? `${this.getDisplayName(languageKey)} (${langConfig.lang_code})`
      : languageKey;
    ...
    try {
      ...
      if (scopedLangCode) {
        try {
          ...
          console.log(
            `Language-scoped shared load skipped for ${scopedLangCode}:`,
            e?.message || e,
          );
        }
      }
      ...
      this.populateDataTable();
      this.setStatus(`Showing ${label}`, "success");
    } catch (error) {
  ...
  setupTableControlListeners() {
    document.querySelectorAll(".sort-btn").forEach((btn) => {
      ...
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        if (!this.currentLanguage) return;
        this.sortTable(btn.dataset.sort);
      });
      ...
      .forEach((btn) => {
        ...
        btn.addEventListener("click", () => {
          ...
          const mode = String(btn.dataset.filter || "")
            .trim()
            .toLowerCase();
          this.applyApprovalFilterForLanguage(this.currentLanguage, mode);
        });
        btn.dataset.bound = "true";
      });
    ...
    if (searchBox && searchBox.dataset.bound !== "true") {
      searchBox.addEventListener("input", (e) => {
        if (!this.currentLanguage) return;
        this.filterTable(this.currentLanguage, e.target.value);
      });
      searchBox.dataset.bound = "true";
    }

    const fileFilter = document.getElementById("fileFilter");
    if (fileFilter && fileFilter.dataset.bound !== "true") {
  ...
  populateDataTable() {
    ...
    const langConfig = this.languages?.[renderLanguage];
    if (!langConfig) {
      this.setStatus(
        "Selected language is no longer configured. Please choose another.",
        "warning",
      );
      return;
    }
    ...
    if (!langCode) {
      this.setStatus(
        `Language "${renderLanguage}" is missing a language code.`,
        "warning",
      );
      return;
    }
    ...
    const previousSignature =
      this.renderSignatureByLanguage.get(renderLanguage);
    if (
      previousSignature === currentSignature &&
      tableContent.children.length > 0
    ) {
      if (typeof setValidationSummaryLoading === "function")
        setValidationSummaryLoading(false);
      if (typeof updateValidationSummary === "function")
      ...
      this.logPerf(
        `Render table skipped (${renderLanguage})`,
        renderStart,
        `rows=${tableContent.children.length}`,
      );
      return;
    }
    const inFlightSignature =
      this.inFlightRenderSignatureByLanguage.get(renderLanguage);
    ...
    this.inFlightRenderSignatureByLanguage.set(
      renderLanguage,
      currentSignature,
    );
    ...
    if (renderLanguage === this.currentLanguage) {
      const quickCounts = this.computeValidationSummaryCountsForRows(
        dataToShow,
        langCode,
        entryCache,
      );
    ...
    function processBatch() {
      if (renderJobId !== self.activeRenderJobId) {
        if (
          self.inFlightRenderSignatureByLanguage.get(renderLanguage) ===
          currentSignature
        ) {
  ...
  buildDataRow(item, index, langCode, entryCache = null) {
    ...
    const displayItemId =
      displayMeta.displayItemId ||
      (String(itemId).includes("::")
        ? String(itemId).split("::").pop()
        : String(itemId));
    const taskName =
      displayMeta.taskName || item.labels || item.task || "general";
    const contentType =
      displayMeta.contentType || item.contentType || "general";
    const originalEnglish = item.en || "No English source";
    row.dataset.itemId = itemId;
    row.dataset.langCode = langCode;
    const escapedItemId = itemId.replace(/'/g, "\\'");
    const escapedOriginal = originalEnglish
      .replace(/'/g, "\\'")
      .replace(/"/g, '\\"');
    ...
    const requiresRevalidation = storedResult?.requiresRevalidation === true;
    const changeKind = String(storedResult?.changeKind || "")
      .trim()
      .toLowerCase();
    ...
    const compactItemId =
      displayMeta.compactItemId ||
      (displayItemIdText.length > 36
        ? `${displayItemIdText.slice(0, 33)}...`
        : displayItemIdText);
    ...
    if (
      ...
      const badgeColor =
        scorePercent >= 85
          ...
          : scorePercent >= 70
            ? "#856404"
            : "#721c24";
      const compositeScore = Number.isFinite(
        Number(storedResult.compositeScore),
      )
        ? Number(storedResult.compositeScore)
        : Number.isFinite(Number(storedResult.baselineScore))
          ? Number(storedResult.baselineScore)
          : null;
      const semanticScore = Number.isFinite(Number(storedResult.semanticScore))
        ? Number(storedResult.semanticScore)
        : null;
      const lexicalScore = Number.isFinite(Number(storedResult.lexicalScore))
        ? Number(storedResult.lexicalScore)
        : null;
      const scoreSource = String(
        storedResult.scoreSource ||
          (storedResult.manualApproved
            ? "manual"
            : storedResult.aiUsed ||
                Number.isFinite(Number(storedResult.aiScore))
              ? "ai"
              : "calculated"),
      ).toLowerCase();
      const tooltipParts = [
      ...
      if (compositeScore != null)
        tooltipParts.push(`Composite: ${compositeScore.toFixed(2)}%`);
      if (semanticScore != null)
        tooltipParts.push(`Semantic: ${semanticScore.toFixed(2)}%`);
      if (lexicalScore != null)
        tooltipParts.push(`Lexical: ${lexicalScore.toFixed(2)}%`);
      if (
        storedResult.aiUsed &&
        Number.isFinite(Number(storedResult.aiScore))
      ) {
        ...
        tooltipParts.push(
          `AI score: ${aiScoreNum.toFixed(2)}%${aiModel ? ` via ${aiModel}` : ""}`,
        );
      }
      if (storedResult.scoringVersion)
        tooltipParts.push(`Scoring version: ${storedResult.scoringVersion}`);
      ...
      if (scoreSource === "manual") {
        ...
        const aiModelName =
          rawAiModelName.toLowerCase() === "gpt-4.1" ? "" : rawAiModelName;
        const aiBadgeText = aiModelName
          ? `AI ${escapeHtml(aiModelName)}`
          : "AI";
        const aiTitle = aiModelName
          ? `AI-refined score via ${escapeHtml(aiModelName)}`
          : "AI-refined score";
    ...
    if (advisory && Number.isFinite(Number(advisory.score))) {
      ...
      const advisoryBg =
        advisoryStatus === "pass"
          ...
          : advisoryStatus === "review"
            ? "#fff8e1"
            : "#fdecea";
      const advisoryLabel = advisoryStatus
        ? advisoryStatus.toUpperCase()
        : "N/A";
      const advisoryDataset = advisory.dataset
        ? String(advisory.dataset).toUpperCase()
        : "";
      const advisoryModel = advisory.model
        ? String(advisory.model)
        : "unknown model";
      const advisoryTitle = [
      ...
      const rowStatusClass =
        advisoryStatus === "pass"
          ? "embedding-row-pass"
          : advisoryStatus === "review"
            ? "embedding-row-review"
            : "embedding-row-fail";
      const advisoryDatasetLabel = advisoryDataset
        ? ` ${escapeHtml(advisoryDataset)}`
        : "";
  ...
  setupSortAndReviewHandlers() {
    ...
    tableContent
      .querySelectorAll(".needs-review-checkbox")
      .forEach((checkbox) => {
        ...
        checkbox.onchange = (e) => {
          ...
          self.validation_results[itemId][preferredLangCode].needsReview =
            isChecked;
          ...
          if (typeof updateValidationSummary === "function")
            updateValidationSummary();
          console.log(
            `📝 Needs Review ${isChecked ? "set" : "cleared"} for ${itemId}[${langCode}]`,
          );
        };
      });
  ...
  selectRow(rowElement, item) {
    ...
    if (!text) {
      ...
      const match = keys.find(
        (k) => k.toLowerCase() === langCode.toLowerCase(),
      );
    ...
    this.setStatus(
      `Selected: ${itemId} - "${text.substring(0, 50)}..."`,
      "success",
    );
  }
    ...
    button.addEventListener("click", () => {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          setMessage(
            `Coordinates acquired: ${latitude.toFixed(4)}, ${longitude.toFixed(4)}. Resolving nearest city...`,
          );
          try {
            ...
            if (payload.results && payload.results.length) {
              ...
              const parts = [best.name, best.admin1, best.country].filter(
                Boolean,
              );
              const locationLine = parts.join(", ");
              setMessage(
                `${locationLine || "Unknown area"} · approx ${best.distanceKm} km away`,
              );
              this.setStatus(
                `Detected nearest city: ${locationLine || "Unknown area"}`,
                "success",
              );
            } else {
              setMessage("No nearby populated place found within 150 km.");
              this.setStatus(
                "No nearby city found for your location.",
                "warning",
              );
            }
          } catch (error) {
  ...
  setupSearchListeners() {
    const searchBox = document.getElementById("searchBox");
    if (!searchBox || searchBox.dataset.bound === "true") return;
    searchBox.addEventListener("input", (e) => {
      if (!this.currentLanguage) return;
      this.filterTable(this.currentLanguage, e.target.value);
    });
  ...
  filterTable(language, searchTerm) {
    ...
    if (searchTerm) {
      this.setStatus(
        `Showing ${visibleCount} items matching "${searchTerm}" in ${this.getDisplayName(language)}`,
        "success",
      );
    }
  }
  ...
  showButtonFeedback(
    ...
    const timeoutIdRaw =
      button.dataset.feedbackTimeout && button.dataset.feedbackTimeout !== ""
...
async function copyItemIdToClipboard(itemId, buttonEl, event) {
  ...
  try {
    ...
    if (
      window.dashboard &&
      typeof window.dashboard.showButtonFeedback === "function"
    ) {
      window.dashboard.showButtonFeedback(
        buttonEl,
        "Copied",
        "success",
        "fa-check",
