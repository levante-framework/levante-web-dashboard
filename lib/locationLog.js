...
async function appendLog(entry) {
  try {
    console.log('location-log: appendLog called with entry:', entry);
    const entries = await readLog();
    console.log(`location-log: Read ${entries.length} existing entries`);
    ...
    console.log('location-log: Attempting to write to GCS...');
    const gcsWritten = await writeLogToGCS(entries);
    if (gcsWritten) {
      console.log(`location-log: Successfully written ${entries.length} entries to GCS bucket`);
      return entries;
    }
    
    console.log('location-log: GCS write failed, trying local file fallback...');
    // Fallback to local file (for development)
    try {
      ensureLogFile();
      ...
      console.warn('location-log: using memory store (filesystem is read-only, GCS unavailable):', writeError.message);
    }
